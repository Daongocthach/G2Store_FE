...
function CardProduct({ product, isShort }) {
  ...
  return (
    <Card sx={{ maxWidth: 300, cursor: 'pointer' }} >
      ...
      <CardContent sx={{ height: 145 }} onClick={() => { navigate('/product-detail', { state: product?.product_id }) }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <Typography variant='body2' sx={{ textAlign: 'center' }} color={'#777777'}> {product?.sold_quantity} Đã bán</Typography>
        </Box>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
          <Typography sx={{ fontSize: 13 }} color={'#777777'}> {product?.shop?.province}</Typography>
      ...
      <CardActions sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {!isShort && <Tooltip title="Giúp đỡ"><Button size="small" sx={{ color: '#333333', ':hover': { bgcolor: 'inherit' } }}><Help sx={{ fontSize: 20 }} /></Button></Tooltip>}
      </CardActions>
