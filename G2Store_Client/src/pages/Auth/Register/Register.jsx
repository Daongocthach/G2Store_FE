...
function Register() {
  ...
  const handleShowOTP = async () => {
    if (!validateEmail(email) || password !== repeatPassword || !captcha) {
      ...
      authenApi.register(captcha, email, password)
        .then(() => {
          triggerAlert('Đã gửi OTP! Vui lòng kiểm tra hộp thư của bạn', false, false)
          setShowOTP(true)
        })
        .catch((error) => {
          console.log(error)
          recaptchaRef.current.reset()
          triggerAlert(error?.response?.data?.message === 'Email existed' ? 'Email đã tồn tại!' : 'Gửi OTP thất bại!', true, false)
        })
  ...
  const onFinish = async () => {
    if (!otp) {
      triggerAlert('Vui lòng nhập otp!', false, true)
    } else {
      setLoading(true)
