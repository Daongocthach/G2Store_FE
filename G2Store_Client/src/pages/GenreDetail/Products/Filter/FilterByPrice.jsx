...
const FilterByPrice = ({ isFilter, setIsFilter, setStartPrice, setEndPrice }) => {
    return (
        <Box mt={2}>
            <Typography variant='subtitle1' color={'#444444'} fontWeight={'bold'} >Lọc theo giá</Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                <input type='text' placeholder='Min' onFocus={(e) => e.target.select()} onChange={(e) => setStartPrice(parseInt(e.target.value))}
                    style={{ border: '1px solid', borderColor: '#CCCCCC', width: 60, borderRadius: 2, height: 30, textAlign: 'center' }} />
                -
                <input type='text' placeholder='Max' onFocus={(e) => e.target.select()} onChange={(e) => setEndPrice(parseInt(e.target.value))}
                    style={{ border: '1px solid', borderColor: '#CCCCCC', width: 60, borderRadius: 2, height: 30, textAlign: 'center' }} />
